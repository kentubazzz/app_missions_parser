...
public class JsonMissionParser implements MissionParser {
    ...
    public Mission parse(File file) throws InvalidMissionFormatException {
        try {
            ...
            Long damage = Long.valueOf(getString(root, "damageCost"));
            if (damage != null) {
            ...
            if (techniquesNode != null && techniquesNode.isArray()) {
                for (JsonNode t : techniquesNode) {
                    ...
                    long techDamage = Long.parseLong(getString(t, "damage"));
                    if (name != null) {
            ...
            String missionFinal = getString(root, "missionFinal");
            if( missionFinal != null){
                mission.addExtension("missionFinal", missionFinal);
            }

            return mission;

        } catch (IOException e) {
            throw new InvalidMissionFormatException("Ошибка парсинга JSON: " + e.getMessage(), e);
        }
    }
